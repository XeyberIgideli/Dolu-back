...
async function getId(mediaName,lang,type) {
    const searchUrl = `https://www.opensubtitles.org/en/search2/moviename-${mediaName}/sublanguageid-${lang}`;
    try {
...
async function getEpisodeSubtitleInfo (mediaId,lang,totalLink) {
    ...
    try {
      const episodeResponse = await axios.get(searchUrl).catch(err => console.log(err))
      const $ = cheerio.load(episodeResponse.data)  
      const arrLinks = []
      let dwLinksEpisodes = [] 

      const smlinks = $('td a[itemprop="url"]').each(async (i, element) => {
       const slug = element.attribs.href.split('-')
      //  console.log(element.attribs.href.match(/\/subtitles\/(\d+)\//))
      if(slug.includes('eng/imdbid')) {
        arrLinks.push(`https://www.opensubtitles.org/en/search/sublanguageid-eng/imdbid-${slug.slice(-1)[0]}`)        
      } 
      })
